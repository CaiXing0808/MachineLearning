...
def plot_learning_curves(model, X, Y,stepsize):
    ...
    for m in range (10000, int(len(X_train)),stepsize):
...
if __name__ == "__main__":
  try: 
    ...
    pred = ANN_income.predict(X_test)
    ...
    print(cross_val_score(ANN_income, X_train, Y_train.values.ravel(), cv=5, scoring = "accuracy"))

    plt=plot_learning_curves(ANN_income, X, Y.values.ravel(),2000)
    plt.savefig("ANN_adult_LearningCurve")
    ##################################################################################
    ## Tuning and Pruning
    #######################################################################################
